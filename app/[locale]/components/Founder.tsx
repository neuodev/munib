    ...
    <section className="min-h-screen bg-white py-16 px-4" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 gap-6">
        ...
        {sections.map((section) => (
          ...
          >
            {/* Section Header */}
            <div className={`flex items-center gap-3 mb-5 transition-all ${locale === 'ar' ? 'flex-row' : ''}`}>
            ...
            <ul className="space-y-3">
              {section.items.map((item, i) => (
                <li key={i} className={`flex items-start gap-3 text-gtext text-sm font-medium leading-relaxed ${locale === 'ar' ? '' : ' text-left'}`}>
